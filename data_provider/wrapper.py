...
def preprocess_item(item):
    ...
    N = x.size(0)  # N是节点数目
    x = convert_to_single_emb(x)  # 给每个节点的第一个属性加1，第二个属性加1+512，第三个属性加1+1024，第四个属性加1+1536...

    # node adj matrix [N, N] bool
    adj = torch.zeros([N, N], dtype=torch.bool)
    adj[edge_index[0, :], edge_index[1, :]] = True  # 构建邻接矩阵

    # edge feature here
    if len(edge_attr.size()) == 1:
        edge_attr = edge_attr[:, None]  # 相当于  edge_attr.unsqueeze(1)
